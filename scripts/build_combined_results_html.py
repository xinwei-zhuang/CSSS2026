...
NORM_DIRS = ["allc", "self", "disc", "sj", "shun", "crit", "mkt", "local"]
ASSETS = [
...
def summary_table(rows: list[dict[str, str]]) -> str:
    ...
    for row in rows:
        body.append(
            ...
            f"<td>{fmt(row['alive_fraction'])}</td>"
            f"<td>{fmt(row['critical_survival'])}</td>"
            f"<td>{fmt(row['annual_mean_served_fraction'])}</td>"
            f"<td>{fmt(row['mean_stress_memory'], 4)}</td>"
            f"<td>{int(float(row['annual_cooperation_successes']))}</td>"
            f"<td>{fmt(row['mean_pool_members'], 1)}</td>"
            "</tr>"
        )
    return "\n".join(body)
    ...
    <section id="{html.escape(norm_dir)}" class="norm-section">
      <div class="section-head">
        <div>
          ...
          <p>Alive {fmt(summary['alive_fraction'])} · Critical survival {fmt(summary['critical_survival'])} · Annual service {fmt(summary['annual_mean_served_fraction'])} · Sharing successes {int(float(summary['annual_cooperation_successes']))}</p>
        </div>
        <a href="#top">Top</a>
      </div>
  ...
  <main id="top">
    <header class="hero">
      <div>
        ...
        <p>Eight fixed sharing rules under the same annual no-grid baseline. All images and GIFs are embedded in this single HTML file.</p>
      </div>
    ...
    <section class="panel">
      <h2>Summary</h2>
      <p>Resilience is read as critical-load survival, annual service fraction, and low stress memory under no external grid support.</p>
      <table>
        <thead>
          <tr>
            <th>Norm</th>
            <th>Name</th>
            <th>Alive</th>
            <th>Critical survival</th>
            <th>Annual service</th>
            <th>Stress memory</th>
            <th>Sharing successes</th>
            <th>Pool members</th>
          </tr>
        </thead>
        <tbody>
