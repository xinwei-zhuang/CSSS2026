...
def write_hierarchy_canopy_png(cells: list[Cell], grid_size: int, path: Path) -> None:
    ...
    def draw_network_canopy(
        ...
        for row, col in network:
            if (row + col) % line_every == 0:
                draw.line([iso(row, col, 0.0), iso(row, col, z)], fill=blend(color, 0.45), width=1)
...
<head>
  ...
  <style>
    ...
    .swatch {{ width: 13px; height: 13px; display: inline-block; margin: 0 4px -2px 8px; border-radius: 3px; border: 1px solid rgba(15,23,42,.16); }}
    .line {{ width: 22px; height: 2px; display: inline-block; margin: 0 4px 3px 8px; background: #172033; }}
  </style>
</head>
<body>
  <main>
    ...
    <div class="panel"><h2>Agreement Components</h2><p>The colored cells are emergent agreement components, not preassigned blocks: <span class="swatch" style="background:#6366f1"></span><strong>purple</strong> is component 1, the largest connected agreement network with 311 buildings; <span class="swatch" style="background:#0e7490"></span><strong>blue-green</strong> is component 2 with 2 buildings; <span class="swatch" style="background:#f59e0b"></span><strong>yellow</strong> is component 3 with 2 buildings. <span class="line"></span><strong>black line segments</strong> are paid adjacent transmission agreements between neighboring buildings.</p><img src="sf_energy_growth_agreement_components.png" alt="agreement components"></div>
